...
void ElectronicStructure::update_populations() { *A = ((*Ccurr).conj()) * ((*Ccurr).T()); }

double ElectronicStructure::norm() { return ((*Ccurr).H() * (*Ccurr)).M[0].real(); }
...
void ElectronicStructure::check_decoherence(
    double dt, int boltz_flag, double Temp, CMATRIX& rates, Random& rnd) {
  ...
  for (int i = 0; i < num_states; i++) {
    ...
    if (t_m[i] >= rnd_i) {  // Decoherence event occurs for state i
      ...
      //        if(boltz_flag==1){
      double dE =
          (Hcurr->M[i * num_states + i] - Hcurr->M[curr_state * num_states + curr_state]).real();
      if (dE > 0) {
        P *= exp(-(dE / (kb * Temp)));
      }  // hop to higher energy state is difficult
         //        }
      ...
      ;
      ;
    }  // Coherence of all states maintained
  }  // for i
  ...
  for (int i = 0; i < num_states; i++) {
    ...
    for (int j = 0; j < num_states; j++) {
      if (j != i) {
        ...
        complex<double> Hij =
            Hcurr->M[i * num_states + j] + Ef.M[0] * Hprimex->M[i * num_states + j] +
            Ef.M[1] * Hprimey->M[i * num_states + j] + Ef.M[2] * Hprimez->M[i * num_states + j];
        double E_i =
            (Hcurr->M[i * num_states + i] + Ef.M[0] * Hprimex->M[i * num_states + i] +
             Ef.M[1] * Hprimex->M[i * num_states + i] + Ef.M[2] * Hprimex->M[i * num_states + i])
                .real();
        double E_j =
            (Hcurr->M[j * num_states + j] + Ef.M[0] * Hprimex->M[j * num_states + j] +
             Ef.M[1] * Hprimex->M[j * num_states + j] + Ef.M[2] * Hprimex->M[j * num_states + j])
                .real();

        double g_ij =
            (2.0 * dt / (a_ii * HBAR)) * (A->M[i * num_states + j] * Hij).imag();  // g_ij = P(i->j)
  ...
  for (int i = 0; i < num_states; i++) {
    for (int j = 0; j < num_states; j++) {
      if (j != i) {
        ...
        g[i * num_states + j] = 1.0;
      }
    }  // for j
  }  // for i
}

void ElectronicStructure::update_hop_prob_fssh(
    double dt, int boltz_flag, double Temp, CMATRIX& Ef, double Eex, CMATRIX& rates) {
  /*******************************************************
...
*******************************************************/
  ...
  CMATRIX* Heff;
  Heff = new CMATRIX(num_states, num_states);
  ...
  for (int i = 0; i < num_states; i++) {
    ...
    for (int j = 0; j < num_states; j++) {
      if (j != i) {
        ...
        g[i * num_states + j] =
            (2.0 * dt / (a_ii * HBAR)) *
            (A->M[i * num_states + j] * Heff->M[i * num_states + j]).imag();  // g_ij = P(i->j)

        if (g[i * num_states + j] < 0.0) {
          g[i * num_states + j] = 0.0;
        }
...
void ElectronicStructure::update_hop_prob_mssh(
    double dt, int boltz_flag, double Temp, CMATRIX& Ef, double Eex, CMATRIX& rates) {
  /*******************************************************
  Here we actually sum up all the transition probabilities
*******************************************************/
...
void ElectronicStructure::update_hop_prob_gfsh(
    double dt, int boltz_flag, double Temp, CMATRIX& Ef, double Eex, CMATRIX& rates) {
  /*******************************************************
 Here we actually sum up all the transition probabilities
*******************************************************/
  ...
  Heff = new CMATRIX(num_states, num_states);
  CMATRIX* C_dot;
  C_dot = new CMATRIX(num_states, 1);
  *Heff = *Hcurr + (Ef.M[0] * (*Hprimex) + Ef.M[1] * (*Hprimey) + Ef.M[2] * (*Hprimez));
  ...
  *C_dot = -one * (*Heff * *Ccurr);  // assume hbar = 1
  ...
  CMATRIX* A_dot;
  A_dot = new CMATRIX(num_states, num_states);
  *A_dot = (*C_dot).conj() * (*Ccurr).T() +
           (*Ccurr).conj() * (*C_dot).T();  // this should be real matrix
...
void ElectronicStructure::propagate_coefficients(double dt, CMATRIX& Ef) {
  ...
  for (i = 0; i < num_states; i++) {
    for (j = i + 1; j < num_states; j++) {
      Hprime = Ef.M[0] * Hprimex->M[i * num_states + j] + Ef.M[1] * Hprimey->M[i * num_states + j] +
  ...
  // exp(iL1 * dt)
  for (i = 0; i < num_states; i++) {
    Hprime = Ef.M[0] * Hprimex->M[i * num_states + i] + Ef.M[1] * Hprimey->M[i * num_states + i] +
...
***********************************************************************/
  ...
  *Hcurr = *Hcurr + dt * (*dHdt);
  if (opt == 1) {
    *Cnext = *Ccurr - (i * dt / HBAR) * (*Hcurr) * (*Ccurr);
  } else if (opt == 2) {
    *Cnext = *Cprev - 2.0 * (i * dt / HBAR) * (*Hcurr) * (*Ccurr);
  }
