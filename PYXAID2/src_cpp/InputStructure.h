...
public:
  std::string Ham_re_prefix;
  int is_Ham_re_prefix;
  std::string Ham_re_suffix;
  int is_Ham_re_suffix;
  std::string Ham_im_prefix;
  int is_Ham_im_prefix;
  std::string Ham_im_suffix;
  int is_Ham_im_suffix;

  std::string Haa_re_prefix;
  int is_Haa_re_prefix;
  std::string Haa_re_suffix;
  int is_Haa_re_suffix;
  std::string Haa_im_prefix;
  int is_Haa_im_prefix;
  std::string Haa_im_suffix;
  int is_Haa_im_suffix;

  std::string Hab_re_prefix;
  int is_Hab_re_prefix;
  std::string Hab_re_suffix;
  int is_Hab_re_suffix;
  std::string Hab_im_prefix;
  int is_Hab_im_prefix;
  std::string Hab_im_suffix;
  int is_Hab_im_suffix;

  std::string Hbb_re_prefix;
  int is_Hbb_re_prefix;
  std::string Hbb_re_suffix;
  int is_Hbb_re_suffix;
  std::string Hbb_im_prefix;
  int is_Hbb_im_prefix;
  std::string Hbb_im_suffix;
  int is_Hbb_im_suffix;

  // only real components are non-zero, so below files are for _re
  std::string Hprime_x_prefix;
  int is_Hprime_x_prefix;
  std::string Hprime_y_prefix;
  int is_Hprime_y_prefix;
  std::string Hprime_z_prefix;
  int is_Hprime_z_prefix;
  std::string Hprime_x_suffix;
  int is_Hprime_x_suffix;
  std::string Hprime_y_suffix;
  int is_Hprime_y_suffix;
  std::string Hprime_z_suffix;
  ...
  std::string energy_in_one_file;
  int is_energy_in_one_file;
  std::string scratch_dir;
  int is_scratch_dir;

  std::string read_couplings;
  int is_read_couplings;
  std::string read_overlaps;
  int is_read_overlaps;
  //  int many_electron_algorithm;  int is_many_electron_algorithm;
  int integrator;
  int is_integrator;  // choose integration algorithm
  double nucl_dt;
  int is_nucl_dt;  // nuclear time step in fs
  double elec_dt;
  int is_elec_dt;  // electronic time step in fs
  int namdtime;
  int is_namdtime;
  int sh_algo;
  int is_sh_algo;  // surface hopping algorithm: 0 = FSSH, 1 = GFSH, 2 = MSSH
  int num_sh_traj;
  int is_num_sh_traj;
  int boltz_flag;
  int is_boltz_flag;
  double Temp;
  int is_Temp;  // Temperature
  int debug_flag;
  int is_debug_flag;
  std::string runtype;
  int is_runtype;
  int alp_bet;
  int is_alp_bet;  // coupling between alpha and beta chanels, 1 - yes, 0 - no
  int decoherence;
  int is_decoherence;  // choose the decoherence method to use; 0 - no decoherence
  int regress_mode;
  int is_regress_mode;  // regression mode used during dephasing times calculations
  int td_pop;
  int is_td_pop;  // whether to output time-dependent population Ci^*Cj, 1 - yes, 0 - no
