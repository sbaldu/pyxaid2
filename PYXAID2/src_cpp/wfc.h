...
public:
  ...
  friend MO operator*(const complex<double>& f,
                      const MO& m1);  // Multiplication of MO and complex<double>;
  friend MO operator*(const MO& m1,
                      const complex<double>& f);  // Multiplication of MO and complex<double>;
  friend MO operator*(const complex<float>& f,
                      const MO& m1);  // Multiplication of MO and complex<float>;
  friend MO operator*(const MO& m1,
                      const complex<float>& f);  // Multiplication of MO and complex<float>;
...
public:
  ...
  K_point(int _nbnds) {
    ...
    npw = 0;
    kx = ky = kz = 0;
    mo = vector<MO>(nbands, MO());
  }
  K_point(int _nbnds, int _npw) {
    nbands = _nbnds;
    npw = _npw;
    kx = ky = kz = 0;
    mo = vector<MO>(nbands, MO(npw));
  }
...
public:
  ...
  wfc(int _nkpts) {
    ...
    nbands = 0;
    npw = 0;
    kpts = vector<K_point>(nkpts, K_point());
    is_allocated = 1;
  }
  wfc(int _nkpts, int _nbnds) {
    ...
    nbands = _nbnds;
    npw = 0;
    kpts = vector<K_point>(nkpts, K_point(nbands));
    is_allocated = 2;
  }
...
void nac(
    wfc& wfc1, wfc& wfc2, int k1, int k2, int minband, int maxband, double dt, std::string filename);
void ham(
    wfc& wfc1, wfc& wfc2, int k1, int k2, int minband, int maxband, double dt, std::string filename);
