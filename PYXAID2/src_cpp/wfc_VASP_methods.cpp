...
void wfc::VASP_read_wfc(std::string filename) {
  ...
  if (file.is_open()) {
    ...
    get_value<double>(inkpt, memblock, pos);
    nkpts = (int)inkpt;
    get_value<double>(inband, memblock, pos);
    nbands = (int)inband;
    ...
    for (int i = 0; i < 9; i++) {
      get_value<double>(A[i], memblock, pos);
    }
    ...
    for (int ikpt = 0; ikpt < nkpts; ikpt++) {
      ...
      get_value<double>(tmp, memblock, pos);
      kpts[ikpt].kx = (int)tmp;
      get_value<double>(tmp, memblock, pos);
      kpts[ikpt].ky = (int)tmp;
      get_value<double>(tmp, memblock, pos);
      kpts[ikpt].kz = (int)tmp;
