...
K_point::K_point(K_point& k1, int min1, int max1, K_point& k2, int min2, int max2) {
  ...
  if (k1.kx != k2.kx) {
    cout << "Error in K_point constructor: Can not merge two k-points with different kx\n";
    exit(0);
  }
  if (k1.ky != k2.ky) {
    cout << "Error in K_point constructor: Can not merge two k-points with different ky\n";
    exit(0);
  }
  if (k1.kz != k2.kz) {
    cout << "Error in K_point constructor: Can not merge two k-points with different kz\n";
    exit(0);
  }
  ...
  if (k1.npw != k2.npw) {
    cout << "Error in K_point constructor: Cannot construct a wfc from the wfcs with different "
            "basis sizes (npw)\n";
    exit(0);
  }
  ...
  if (max1 >= min2) {
    cout << "Warning in K_point constructor: There are several identical bands (MO) in given "
            "K_point\n";
  }
...
wfc::wfc(wfc& w1, int min1, int max1, wfc& w2, int min2, int max2) {
  ...
  if (w1.npw != w2.npw) {
    cout << "Error in wfc constructor: Cannot construct a wfc from the wfcs with different basis "
            "sizes\n";
    exit(0);
  }
  npw = w1.npw;
...
void wfc::transform(int k, CMATRIX& T) { kpts[k].transform(T); }
...
void wfc::set_latt_vectors(boost::python::list lst) {
  ...
  for (int i = 0; i < sz; i++) {
    tmp[i] = boost::python::extract<double>(lst[i]);
  }

  if (sz >= 9) {
    a1 = vector<double>(3, 0.0);
    a1[0] = tmp[0];
    a1[1] = tmp[1];
    a1[2] = tmp[2];
    a2 = vector<double>(3, 0.0);
    a2[0] = tmp[3];
    a2[1] = tmp[4];
    a2[2] = tmp[5];
    a3 = vector<double>(3, 0.0);
    a3[0] = tmp[6];
    a3[1] = tmp[7];
    a3[2] = tmp[8];

  } else {
    cout << "Error: in set_latt_vectors. Too few items in the list : " << sz
         << " Expected at least 9\n";
    exit(0);
  }
}
...
void wfc::set_reci_vectors(boost::python::list lst) {
  ...
  if (sz >= 9) {
    b1 = vector<double>(3, 0.0);
    b1[0] = tmp[0];
    b1[1] = tmp[1];
    b1[2] = tmp[2];
    b2 = vector<double>(3, 0.0);
    b2[0] = tmp[3];
    b2[1] = tmp[4];
    b2[2] = tmp[5];
    b3 = vector<double>(3, 0.0);
    b3[0] = tmp[6];
    b3[1] = tmp[7];
    b3[2] = tmp[8];
...
void wfc::compute_Hprime(int minband, int maxband, std::string filename) {
  ...
  int is_compl = 0;
  if (minband < 0) {
    cout << "Error in compute_Hprime: minband<0 : minband = " << minband << endl;
    exit(0);
  }
  if (maxband >= nbands) {
    cout << "Error in compute_Hprime: maxband>=nbands : maxband = " << maxband
         << " nbands = " << nbands << endl;
    exit(0);
  }
  if (g_sz != npw) {
  ...
  if (b1.size() < 3) {
    cout << "Error in compute_Hprime: b1 vector is not defined\n";
    exit(0);
  }
  if (b2.size() < 3) {
    cout << "Error in compute_Hprime: b2 vector is not defined\n";
    exit(0);
  }
  if (b3.size() < 3) {
    cout << "Error in compute_Hprime: b3 vector is not defined\n";
    exit(0);
  }
      ...
      for (int g = 0; g < g_sz; g++) {
        ...
        if (is_compl == 0) {
          ...
          // Now the Hprime_ matrices are purely imaginary, for the case of gamma-symmetry.
            ...
            Hx += 2.0 * scl1 * tmp.real() * gx;
            Hy += 2.0 * scl1 * tmp.real() * gy;
            Hz += 2.0 * scl1 * tmp.real() * gz;
          }
        }
