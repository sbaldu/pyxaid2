...
int namd(boost::python::dict inp_params) {
  ...
  input_states(
      inp_params,
      me_states);  // Convention will be to count from 1, where 1 - is the first orbital from input
  // (real and energy files), not necessarily the actual first orbital of the system

  int me_numstates =
      me_states.size();  // This is a number of true (multi-electron) states to propagate
  int numstates =
      me_states[0].active_space.size();  // This is a number of 1-electron orbitals to propagate
  int num_elec =
      me_states[0].actual_state.size();  // This is a number of valent (capable to hop) electrons
  ...
  complex<double> hp_scl(
      1.0,
      0.0);  // = -ihbar; This is because I initially overlooked i (complex unity) in p operator
  ...
  cout << "Maximal Hamiltonian file to read is " << params.Ham_re_prefix << (max_indx + 1)
       << params.Ham_re_suffix << endl;
  ...
  if (params.read_couplings == "batch" || params.read_couplings == "batch_all_in_one") {
    ...
    for (int j = 0; j < max_indx; j++) {
      ...
      Ham_re_file = params.Ham_re_prefix + int2str(j) + params.Ham_re_suffix;
      std::string Haa_re_file;
      Haa_re_file = params.Haa_re_prefix + int2str(j) + params.Haa_re_suffix;
      std::string Hab_re_file;
      Hab_re_file = params.Hab_re_prefix + int2str(j) + params.Hab_re_suffix;
      std::string Hbb_re_file;
      Hbb_re_file = params.Hbb_re_prefix + int2str(j) + params.Hbb_re_suffix;
      ...
      extract_2D(Ham_re,
                 Ham_re_crop,
                 me_states[0].active_space,
                 -1);  // also crop the matrix to the active space
      ...
      vector<vector<double> > Ham_im, Ham_im_crop, Haa_im, Haa_im_crop, Hab_im, Hab_im_crop, Hbb_im,
          Hbb_im_crop;
      std::string Ham_im_file;
      Ham_im_file = params.Ham_im_prefix + int2str(j) + params.Ham_im_suffix;
      std::string Haa_im_file;
      Haa_im_file = params.Haa_im_prefix + int2str(j) + params.Haa_im_suffix;
      std::string Hab_im_file;
      Hab_im_file = params.Hab_im_prefix + int2str(j) + params.Hab_im_suffix;
      std::string Hbb_im_file;
      Hbb_im_file = params.Hbb_im_prefix + int2str(j) + params.Hbb_im_suffix;
      ...
      if (params.is_field == 1) {
        ...
        Hprime_x_file = params.Hprime_x_prefix + int2str(j) + params.Hprime_x_suffix;
        std::string Hprime_y_file;
        Hprime_y_file = params.Hprime_y_prefix + int2str(j) + params.Hprime_y_suffix;
        std::string Hprime_z_file;
        Hprime_z_file = params.Hprime_z_prefix + int2str(j) + params.Hprime_z_suffix;
        ...
        vector<vector<double> > tmp(Hprime_x_crop.size(),
                                    vector<double>(Hprime_x_crop.size(), 0.0));
        //------------ Create matrix --------
  ...
  for (icond = 0; icond < iconds.size(); icond++) {  // first_icond may start from 0, not 1
    ...
    vector<ElectronicStructure> oe_es(params.namdtime,
                                      ElectronicStructure(2 * numstates));  // one-electron orbitals
    vector<ElectronicStructure> me_es(
        params.namdtime, ElectronicStructure(me_numstates));  // multi-electron orbitals

    for (int j = iconds[icond][0]; j < iconds[icond][0] + params.namdtime;
         j++) {  // namdtime also starts from 0
      if (params.debug_flag == 1) {
        cout << "----------- j = " << j << " -------------------" << endl;
      }
      ...
      if (params.read_couplings == "online" || params.read_couplings == "online_all_in_one") {
        ...
        *T = Ham;
        ...
        if (params.is_field == 1) {
          ...
          Tx = new CMATRIX(Hprimex.n_rows, Hprimex.n_cols);
          *Tx = Hprimex;
          Ty = new CMATRIX(Hprimey.n_rows, Hprimey.n_cols);
          *Ty = Hprimey;
          Tz = new CMATRIX(Hprimez.n_rows, Hprimez.n_cols);
          *Tz = Hprimez;
        ...
        T = new CMATRIX(H_batch[j].n_rows, H_batch[j].n_cols);
        *T = H_batch[j];
        Taa = new CMATRIX(Haa_batch[j].n_rows, Haa_batch[j].n_cols);
        *Taa = Haa_batch[j];
        Tab = new CMATRIX(Hab_batch[j].n_rows, Hab_batch[j].n_cols);
        *Tab = Hab_batch[j];
        Tbb = new CMATRIX(Hbb_batch[j].n_rows, Hbb_batch[j].n_cols);
        *Tbb = Hbb_batch[j];

        if (params.is_field == 1) {
          Tx = new CMATRIX(Hprime_x_batch[j].n_rows, Hprime_x_batch[j].n_cols);
          *Tx = Hprime_x_batch[j];
          Ty = new CMATRIX(Hprime_y_batch[j].n_rows, Hprime_y_batch[j].n_cols);
          *Ty = Hprime_y_batch[j];
          Tz = new CMATRIX(Hprime_z_batch[j].n_rows, Hprime_z_batch[j].n_cols);
          *Tz = Hprime_z_batch[j];
        }  // if params.is_field==1
      }  // "batch"

      CMATRIX Hij(T->n_rows, T->n_cols);
      Hij = *T;
      CMATRIX Haa(Taa->n_rows, Taa->n_cols);
      Haa = *Taa;
      CMATRIX Hab(Tab->n_rows, Tab->n_cols);
      Hab = *Tab;
      CMATRIX Hbb(Tbb->n_rows, Tbb->n_cols);
      Hbb = *Tbb;
      CMATRIX Hij_prime_x(T->n_rows, T->n_cols);
      Hij_prime_x = 0.0;
      CMATRIX Hij_prime_y(T->n_rows, T->n_cols);
      Hij_prime_y = 0.0;
      CMATRIX Hij_prime_z(T->n_rows, T->n_cols);
      Hij_prime_z = 0.0;
