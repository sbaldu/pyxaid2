  ...
  if (file.is_open()) {
    ...
    for (int ikpt = 0; ikpt < nkpts; ikpt++) {
      ...
      for (int iband = 0; iband < kpts[ikpt].nbands; iband++) {
        ...
        kpts[ikpt].mo[iband].coeff =
            std::vector<complex<double> >(kpts[ikpt].mo[iband].npw, std::complex<double>(0.0, 0.0));
        for (int i = 0; i < kpts[ikpt].mo[iband].npw; i++) {
          get_value<std::complex<double> >(c, memblock, pos);
          kpts[ikpt].mo[iband].coeff[i] = c;
        }
...
void wfc::QE_read_acsii_wfc(std::string filename) {
  ...
  for (int k = 0; k < nkpts; k++) {
    ...
    if (verbose) {
      cout << "Number of bands for k-point " << k << " is = " << knbnd << endl;
    }
  }  // for k
...
void wfc::QE_read_acsii_index(std::string filename) {
  ...
  if (status) {
    for (int i = beg; i < end; i++) {
      pos = A[i].find("<Kpoints");
      if (pos != string::npos) {
        str = extract_s(A[i], "nktot");
        nkpts = atoi(str.c_str());
      }
      pos = A[i].find("<Kpoints");
      if (pos != string::npos) {
        str = extract_s(A[i], "nspin");
        nspin = atoi(str.c_str());
      }
      pos = A[i].find("<Wfc_grid");
      if (pos != string::npos) {
        str = extract_s(A[i], "npwx");
        npw = atoi(str.c_str());
      }
      pos = A[i].find("<Bands");
      if (pos != string::npos) {
        str = extract_s(A[i], "nbnd");
        nbands = atoi(str.c_str());
      }
      pos = A[i].find("<Gamma_tricks");
      if (pos != string::npos) {
        str = extract_s(A[i], "gamma_only");
        if (str == "F") {
          gamma_only = 0;
        } else {
          gamma_only = 1;
      ...
      pos = A[i].find("<Atoms");
      if (pos != string::npos) {
        str = extract_s(A[i], "natoms");
        natoms = atoi(str.c_str());
      }
    }  // for i
  }  // if status
  ...
  if (status) {
    for (int i = beg; i < end; i++) {
      pos = A[i].find("<Cell");
      if (pos != string::npos) {
        str = extract_s(A[i], "units");
        cell_units = str;
      }
      pos = A[i].find("<Data");
      if (pos != string::npos) {
        str = extract_s(A[i], "alat");
        alat = atof(str.c_str());
      }
      pos = A[i].find("<Data");
      if (pos != string::npos) {
        str = extract_s(A[i], "omega");
        omega = atof(str.c_str());
      }
      pos = A[i].find("<Data");
      if (pos != string::npos) {
        str = extract_s(A[i], "tpiba");
        tpiba = atof(str.c_str());
      }

      pos = A[i].find("<a1");
      if (pos != string::npos) {
        str = extract_s(A[i], "xyz");
        aux_line2vec(str, a1);
      }
      pos = A[i].find("<a2");
      if (pos != string::npos) {
  ...
  if (status) {
    for (int i = beg; i < end; i++) {
      pos = A[i].find("<Eigenvalues");
      if (pos != string::npos) {
        str = extract_s(A[i], "efermi");
        efermi = atof(str.c_str());
      }
      pos = A[i].find("<Eigenvalues");
      if (pos != string::npos) {
        energy_units = extract_s(A[i], "units");
      }
    }  // for i
  ...
  if (is_allocated == 3) {
    for (int k = 0; k < nkpts; k++) {
      ...
      status =
          find_section(A, "<e." + int2str(k + 1), "</e." + int2str(k + 1), beg, end, beg1, end1);
