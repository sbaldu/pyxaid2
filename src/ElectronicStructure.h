...
public:
  ...
  void set_state(int indx) {
    for (int i = 0; i < num_states; i++) {
      if (i == indx) {
        Ccurr->M[i] = complex<double>(1.0, 0.0);
      } else {
        Ccurr->M[i] = 0.0;
      }
    }
  ...
  void update_hop_prob_fssh(
      double dt, int is_boltz_flag, double Temp, CMATRIX& Ef, double Ex, CMATRIX&);
