...
void hop(std::vector<double>& sh_prob, int& hopstate, int numstates) {
  /***********************************************
...
************************************************/
  ...
  auto in = hopstate;  // initial state
  auto hstate = -1;    // set to an absurd value, so that run fails explicitly if the
                       // surface hopping probabilities are stange
  ksi = rand() / static_cast<double>(RAND_MAX);
...
void regression(std::vector<double>& X, std::vector<double>& Y, int opt, double& a, double& b) {
  // Linear regression
  // opt = 0:   Y =     b*X
  // opt = 1:   Y = a + b*X
  if (X.size() != Y.size()) {
    std::cout << "Error in regression: Size of X array is different from that of Y\n";
    exit(0);
  }

  auto sz = X.size();
  auto x = 0., y = 0., xy = 0., x2 = 0., y2 = 0., N = 0.;

  std::for_each(std::views::zip(X.begin(), Y.begin()), [=](auto xi, auto yi) -> void {
    x += xi;
    y += yi;
    if (opt == 1) {
      x2 += xi * xi;
...
double decoherence_rates(std::vector<double>& x, double dt, std::string rt_dir, int regress_mode) {
  /***********************************************
 Computes:
...
***********************************************/
  auto len = x.size();
  auto sz = (len % 2 == 0) ? (len / 2) : ((len - 1) / 2);

  std::vector<double> C(sz, 0.);    // autocorrelation function
  std::vector<double> IC(sz, 0.);   // first cumulant
  std::vector<double> IIC(sz, 0.);  // second cumulant
  std::vector<double> D(sz, 0.);    // decoherence function
  std::vector<double> T, selIIC;    // for regression
  ...
  std::vector<double> J(Npoints, 0.0);
...
void propagate_electronic(InputStructure& is,
                          std::vector<ElectronicStructure>& es,
                          int i,
                          CMATRIX& rates) {
  ...
  for (int j = 0; j < nel; j++) {
    ...
    if (is.integrator == 0) {
      ...
      if (is.decoherence == 5) {
        cout << "Error: CPF is not available with the integrator=2\nExiting...\n";
        exit(0);
      }
...
void run_decoherence_rates(InputStructure& is,
  ...
  CMATRIX rij(N, N);
  ofstream out((is.scratch_dir + "/decoherence_rates_icond" + int2str(icond) + ".txt").c_str(),
               ios::out);
  ...
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < N; j++) {
      if (i == j) {
        ...
        rij.M[i * N + j] = decoherence_rates(
            Eij,
            is.nucl_dt,
            is.scratch_dir + "/icond" + int2str(icond) + "pair" + int2str(i) + "_" + int2str(j),
            is.regress_mode);
      }
...
void run_namd1(InputStructure& is,
  ...
  std::vector<double> tmp(nst, 0.0);
  std::vector<std::vector<double> > sh_pops(sz, tmp);
  sh_pops[0][curr_state] = 0.0;
  std::vector<std::vector<double> > se_pops(sz, tmp);
  se_pops[0][curr_state] = 0.0;

  // Decoherence stuff
  std::vector<std::vector<double> > r_ij;
  std::vector<std::vector<double> > z(
      nst, std::vector<double>(nst, 0.0));  // 2D matrix with all components set to 0.0

  std::vector<std::vector<double> > E0(nst, std::vector<double>(nst, 0.0));  // average
  std::vector<std::vector<double> > d2E_av(
      nst, std::vector<double>(nst, 0.0));  // average fluctuation of i-j pair
  std::vector<std::vector<std::vector<double> > > d2E(
      sz, std::vector<std::vector<double> >(nst, std::vector<double>(nst, 0.0)));  //fluctuation
  CMATRIX rates(nst, nst);

  std::vector<CMATRIX> sumCij;
  for (int k = 0; k < sz; k++) {
  ...
  if (is.decoherence > 0) {
    ...
    if (is.decoherence == 2) {
      ;
      ;
    }  //is.decoherence == 2 NAC scaling (test method)
    if (is.decoherence == 3) {
      ;
      ;
    }  //is.decoherence == 3 NAC scaling - spectral density variant (test method)
    if (is.decoherence == 4) {
      ;
      ;
    }  //is.decoherence == 4 NAC scaling (same as decoherence==2, but with extrapolation) (test method)
